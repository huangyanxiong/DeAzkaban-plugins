...
public class HadoopSecureHiveWrapper {
  ...
  private static String replacedHiveScript;
  ...
  public static void main(final String[] args) throws Exception {
    ...
    replacedHiveScript = hiveScript+".replaced";

    //支持hiveScript中变量替换 by dataeye
    hiveSqlVariableSubstitution(jobProps);
  ...
  public static void runHive(String[] args) throws Exception {
    ...
    logger.info("Executing query: " + replacedHiveScript);
    ...
    int returnCode = cli.processFile(replacedHiveScript);
    if (returnCode != 0) {
      logger.warn("Got exception " + returnCode + " from line: " + replacedHiveScript);
      throw new HiveQueryExecutionException(returnCode, replacedHiveScript);
    }
  }
  ...
  /**
   * 对hiveSql中变量进行替换,变量必须在job的配置文件中有定义
   *
   * by dataeye
   */
  private static void hiveSqlVariableSubstitution(Properties jobProps) throws IOException {
    String sql = FileUtils.readFileToString(new File(hiveScript));
    ...
    while (matcher.find()) {
      ...
      String replacement = jobProps.getProperty(variable);
      if (replacement == null) {

        throw new RuntimeException("The variable " + variable + " not defined in job properties.");
      }
    ...
    FileUtils.writeStringToFile(new File(replacedHiveScript), replacedSql.toString());
  }
}
